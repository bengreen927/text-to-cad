...
engineering drawings and/or a STEP model of it. The workflow applies to any object: a consumer
product, a machine part, a fixture, an enclosure. It was proven on a multi-part device with molded
parts, rods, springs and fasteners, and it runs in this order:
...
8. run `scripts/inspect interfere` on the assembly and add in-generator checks (`cad-as-config.md`):
   reviewers and snapshots judge what is visible, interference finds what is hidden.
...
Keep the photographs, renders, measurement overlays and any product-identifying material in the
project folder, never in a shared skills repository; the measurement log references them by path.
...
- Note which photos share a camera setup (same session, same table): their scales are often equal,
  which lets a fastener or shaft photographed twice tie two photos together.
...
- Derive each photo's scale from an anchor that appears in it and is already known: the largest
  part from the ruler photo, then a hole pitch from that part's photo, then a fastener photo from the
  screw head, and so on. Write the chain down; a wrong link shows up as a 10 to 20 percent
  disagreement later.
...
- Colour-segment what has a distinct colour (coloured parts, dark shafts) with HSV thresholds and
  use connected components for bounding boxes and centroids. Light parts on a light table cannot be
...
too (hole coordinates, feature counts per part variant, which end carries a thread).
...
plan view, a wrong feature count, a tapered face drawn as a step) are expensive to find in a
...
- Model the pose the photos show most often and add joints for the other poses rather than
  modelling an unphotographed pose.
...
- A `RevoluteJoint`/`LinearJoint` on the fixed part pairs with a `RigidJoint` on the moving part whose
  location matches the same axis. Use explicit `angle=0` for revolute joints and
  `position=0` for linear joints, with zero inside the permitted range. Matching frames
  and an explicit zero displacement are necessary to preserve the authored pose.
- Inspect generator targets during iteration, then separately export and re-import the delivered STEP.
  Compare geometry and critical interfaces after translation; diagnose stale caches without editing the design outputs.
...
## 8. Interference and checks

A reverse-engineered assembly is where hidden overlaps collect: bores that were never modelled because
no photo showed them, an axle drawn through the parts it should pass beside, a spring sitting inside
solid material. Run `scripts/inspect interfere <assembly>.step.py` after the reviews and treat every
pair as a defect or a decision. Then add `checks()` to the generator (see `cad-as-config.md`) for the
pairs that define the mechanism: rods inside their bores, fasteners inside their holes, moving parts
clear of the housing at the modelled pose. Verify errors, coverage, and required operating states;
static validity and visual similarity alone do not establish fit.

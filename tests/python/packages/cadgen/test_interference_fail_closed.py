...
class InterferenceFailClosedTest(unittest.TestCase):
    ...
    def inspect(self, occurrences: list[Occurrence], **kwargs: Any) -> dict:
        ...
        with (
            ...
            patch("cadgen.step_export_target._resolve_spec_and_scene", return_value=SimpleNamespace(scene=object())),
    ...
    def test_malformed_or_inverted_bounds_fail_closed_before_bbox_rejection(self) -> None:
        for bounds in ((0, 0, 0), (2, 0, 0, 1, 1, 1), (None, 0, 0, 1, 1, 1)):
            with self.subTest(bounds=bounds):
                invalid = replace(self.a, bbox=bounds)
                self.assert_incomplete(self.inspect([invalid, self.far]))

    def test_invalid_options_fail_closed_even_without_pairs(self) -> None:
        for tolerance in (float("nan"), float("inf"), float("-inf"), -1.0, "bad", None, True):
            with self.subTest(tolerance=tolerance):
                self.assert_incomplete(self.inspect([], tolerance=tolerance))
                with self.assertRaises(ValueError):
                    find_clashes([], tolerance=tolerance)
        for limit in (-1, float("nan"), float("inf"), 0.5, "bad", True):
    ...
    def test_empty_selection_is_incomplete_but_single_occurrence_is_inconclusive(self) -> None:
    ...
    def test_nonexistent_numeric_selection_is_incomplete(self) -> None:
        result = self.inspect([self.a, self.b], refs=["o99"])
        self.assert_incomplete(result)
        self.assertFalse(result["conclusive"])
        self.assertEqual(result["stats"]["occurrences"], 0)

    def test_intra_part_overlap_remains_complete_and_passes_with_a_clean_neighbor(self) -> None:
        case = replace(self.a, ref="o1.1.1")
        motor = replace(self.b, ref="o1.1.2")
        neighbor = replace(self.far, ref="o1.2")
        result = self.inspect([case, motor, neighbor])
        self.assertTrue(result["complete"])
        self.assertTrue(result["conclusive"])
        self.assertTrue(result["ok"])
        self.assertEqual(result["clashes"], [])
        self.assertEqual(result["intraPartOverlapCount"], 1)
        self.assertEqual(result["intraPartOverlaps"][0]["part"]["ref"], "o1.1")
        selected = self.inspect([case, motor, neighbor], refs=["o1.1"])
        self.assertTrue(selected["complete"])
        self.assertFalse(selected["ok"])
        self.assertEqual(selected["clashCount"], 1)

    def test_failed_or_truncated_intra_part_pairs_are_incomplete(self) -> None:
        bodies = [replace(self.a, ref="o1.1.1"), replace(self.b, ref="o1.1.2"),
                  replace(self.far, ref="o1.2")]
        with patch("cadgen.interference._intersection", return_value=None):
            failed = self.inspect(bodies)
        self.assert_incomplete(failed)
        self.assertEqual(failed["stats"]["pairs_intra_part"], 1)
        self.assertEqual(failed["stats"]["pairs_failed"], 1)
        truncated = self.inspect(bodies, max_pairs=0)
        self.assert_incomplete(truncated)
        self.assertEqual(truncated["stats"]["pairs_truncated"], 1)
